...
prove_aux(p(inv,P,A,_Args,Atom,Path),FullSig,Sig1,MaxN,N1,N2,Prog1,Prog2):-
    ...
    user:metarule_init(Name,MetaSub,PredTypes,(Atom:-Body1),_Recursive,[Atom|Path]),
...
prove_aux(p(inv,P,A,_Args,Atom,Path),FullSig,Sig1,MaxN,N1,N2,Prog1,Prog2):-
    ...
    user:metarule(Name,MetaSub,PredTypes,(Atom:-Body1),FullSig,_Recursive,[Atom|Path]),
    %% (Recursive==true -> \+memberchk(Atom,Path); true),
...
metasub_to_clause_list(sub(false,P,A,[P],_),[HeadList|BodyAsList1]):-!,
    length(Args,A),
    HeadList=[P|Args],
    BodyAsList1=[[false]].
